...
学习 Kubernetes，先不要急着背 `kubectl` 命令。
...
在个人项目里，这样完全可以。但服务一旦变重要，就会不断冒出问题：
...
Kubernetes 想解决的，就是把“盯机器、盯进程、手动修复”变成“声明目标、自动调度、自动恢复”。
...
Kubernetes 的思路不一样。它更像给系统写一张“目标清单”：
...
这就是“声明式”的价值：用户重点描述结果，系统负责追结果。

## 一个最小的 Deployment 示例

下面这段 YAML 不需要一次看懂所有字段，先看它表达的意思。
...
Kubernetes 很重要的一个概念是控制循环。可以把它想成一个值班员，不断做三件事：
...
因为有些容器天然需要绑在一起运行。Pod 提供了一个更稳定的抽象：同一个 Pod 里的容器一起调度、共享网络、生命周期更接近。
...
Pod 会重建，IP 也可能变化。如果其他服务直接访问 Pod IP，就像把朋友家地址记成“今天住在哪个酒店房间”。一旦换房间，地址就失效了。
...
下一篇可以继续拆 Kubernetes 的集群架构，重点看看控制面里的几个组件分别负责什么。
