...
我觉得学习 Kubernetes 最容易卡住的地方，不是某一条命令，也不是某一个 YAML 字段，而是它把很多事情都抽象成了“对象”。

你刚开始看 Kubernetes 的时候，可以把它想成第一次走进一个大型园区：有门禁、有调度中心、有楼栋、有房间、有物业、有告示牌、有维修人员。每个东西单独看都不难，但如果不知道它们之间怎么配合，就会觉得概念很多、关系很乱。

所以这个系列我不打算一上来就带你背命令，而是先把这座“园区”的地图画出来。
...
我们的学习目标很简单：先知道每个对象为什么存在，再看它和其他对象怎么配合。

## 第一阶段：先看懂基本地图

第一阶段我们只解决一个问题：Kubernetes 到底在管理什么？
...
这一阶段我们不追求掌握所有字段。重点是理解一句话：“我声明想要什么，Kubernetes 负责让它尽量变成现实”。

## 第二阶段：理解应用怎么跑稳

应用跑起来只是第一步，长期稳定运行才是关键。所以第二阶段，我们要看应用跑起来之后还会遇到哪些问题。
...
你可以把 Label 当成门牌号，把 Selector 当成查找规则。理解这一点，后面看 Service、Deployment、NetworkPolicy 都会轻松很多。

## 第三阶段：理解发布和故障处理

真实应用不会一直停在“能运行”这个状态。它会升级、扩容、宕机、迁移，也会遇到流量变化。第三阶段我们就专门看这些变化。
...
每篇文章我都尽量只解决一个核心问题。遇到新对象时，我们先问三个问题：

1. 它解决什么麻烦？
2. 如果没有它，会发生什么问题？
3. 它通过什么方式和其他对象配合？

例如学习 Service，不要一上来背 `ClusterIP`、`NodePort`、`LoadBalancer`。我们先理解一个更朴素的问题：Pod 会重建，IP 会变，那调用方该访问谁？
...
后续文章我会按这条路线展开。第一篇先从最基础的问题开始：Kubernetes 到底解决什么问题？
